...
import com.etiosoftware.config.KafkaProperties;
...
class TriadJKafkaResourceIT {
    ...
    void setup() {
        ...
        TriadJKafkaResource kafkaResource = new TriadJKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/triad-j-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/triad-j-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
